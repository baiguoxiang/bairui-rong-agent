...
async function sendMessage() {
  ...
  try {
    ...
    if (data.model) {
      ...
      modelEl.textContent = data.model === "fallback" ? "📚 本地知识库" : "🤖 AI模型: agnes-2.0-flash";
...
async function checkCompliance() {
  ...
  try {
    ...
    if (data.isClean) {
      ...
      resultEl.innerHTML = `<div class="compliance-result compliance-violation">⚠️ 发现 ${data.violations.length} 处疑似违规：<ul>${violations}</ul></div>`;
    }
  } catch (err) {
